...
import struct
import subprocess
from src.log_hotplug import logger as log
...
PCI_EXP_SLOT_CAP_HotPlug_Surprise = 0x0020
PCI_EXP_SLOT_CAP_HotPlug_CAP = 0x0040
...
PCIExpressSlot = namedtuple('PCIExpressSlot',
                            ('slot', 'presence', 'power', 'attn_led','hot_plug_surprise', 'hot_plug_cap'))
...
class PCIDevice(namedtuple('PCIDevice', ('device_name', 'vendor_id',
    ...
    def express_slot(self):
        ...
        with closing(PCIConfigSpace.get(self.device_name)) as config:
            ...
            hot_plug_surprise = (slotcap & PCI_EXP_SLOT_CAP_HotPlug_Surprise) >> get_flag_offset(PCI_EXP_SLOT_CAP_HotPlug_Surprise)
            hot_plug_cap = (slotcap & PCI_EXP_SLOT_CAP_HotPlug_CAP) >> get_flag_offset(PCI_EXP_SLOT_CAP_HotPlug_CAP)

            power = None
            if (slotcap & PCI_EXP_SLOT_CAP_POWER) != 0:
                # 1 is powered -off-, 0 is powered -on-
                power = (slotctl & PCI_EXP_SLOT_CTL_POWER) == 0
            return PCIExpressSlot(slotnum, presence, power, attn_led, hot_plug_surprise, hot_plug_cap)
